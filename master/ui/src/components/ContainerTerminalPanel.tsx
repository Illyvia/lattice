...
export default function ContainerTerminalPanel({
  nodeId,
  containerId,
  containerName,
  apiBaseUrl,
  ...
  const wsUrl = useMemo(
    () => buildTerminalWsUrl(apiBaseUrl, nodeId, containerId, DEFAULT_COLS, DEFAULT_ROWS),
  ...
  useEffect(() => {
    ...
    socket.onmessage = (event) => {
      ...
      if (message.type === "terminal_data") {
        if (typeof message.data === "string") {
          terminalInstance.write(normalizeTerminalChunk(message.data));
        }
        return;
      }
      if (message.type === "terminal_exit") {
        const code = typeof message.exit_code === "number" ? message.exit_code : 0;
        toast.info(`Container terminal closed (exit ${code})`);
        return;
      }
      if (message.type === "terminal_error") {
        const detail = message.error || "terminal error";
        if (detail.includes("is not running") || detail.includes("not found")) {
          disableAutoReconnectRef.current = true;
        }
        toast.error(`Container terminal error: ${detail}`);
      }
    };

    socket.onerror = () => {
      if (!closedByClient) {
        toast.error("Container terminal connection error");
      }
    };

    const scheduleReconnect = () => {
      if (closedByClient || isDisposed || reconnectScheduledRef.current || disableAutoReconnectRef.current) {
        return;
      }
    ...
    socket.onclose = (event: CloseEvent) => {
      ...
      setSessionId(null);
      if (!closedByClient) {
        toast.info("Container terminal disconnected");
      }
  ...
  return (
    <section className="terminal-card">
      <div className="terminal-header">
        <h3>
          <FontAwesomeIcon icon={faTerminal} /> Container Terminal
        </h3>
      ...
      <p className="muted">
        Shell session for {containerName} {sessionId ? `(session ${sessionId.slice(0, 8)})` : ""}.
      </p>
